...
- 7월 15%(2개이상): 신규 렌탈 2대 이상 → 자동 15% (토글 무관)
- 결합할인: 사용자가 5% 결합할인 토글 ON → 신규 렌탈 1대에서 5% 적용
- 반값할인/동시할인: 7월 정책에서는 적용하지 않음
...
- **totalDiff = totalRent - currentTotalRent** (월차액 × 개월수가 아님)
...
- 항목: 자동이체 / 현장할인 / VIP 약정할인 / 루네어 특별할인 / 7월 15%(2개이상) / 결합할인 5% / 카드
- 케어 옵션에 따라 자동 표시/숨김
...
- 할인 항목: 자동이체 + 현장 + VIP + 루네어특별(mLE) + 7월 15%(2개이상)/결합할인 5% + 카드
