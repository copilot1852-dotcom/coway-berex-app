...
`getRentalBenefitDecision`은 7월 정책 기준으로 혜택 객체를 반환한다.
...
`hasExistingCoway`는 7월 정책에서 결합할인 토글 ON 여부만 의미한다.

```js
const hasExistingCoway = !!isCross;
```

기존 `aprilCount`는 7월 프로모션 계산에 사용하지 않는다. 사용자가 `5% 결합할인` 토글을 ON 했을 때만 5%가 적용되어야 하므로, 과거 보유 수량 state가 자동으로 5%를 켜면 안 된다.
...
- `6월 동시+반값`
- `2대 이상 10% 할인과 약정별 반값 혜택을 함께 적용합니다.`
- `10% + 반값 혜택 + 자동이체 1,000원`
